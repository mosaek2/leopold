...
export default function InterestTable({
  ...
  function handleChangeCheckBox(e) {
    if (e.target.checked === true) {
      ...
      const newList = selectedUidList?.filter(
        (selectedUid) => selectedUid !== interest?.uid,
      );
      console.log(newList);
    ...
    <div className="interest-table">
      ...
      <div className="interest-table-td2" style={{ width: 581 }}>
        <p
          ...
          onClick={() => {
            navigate(`/shopping/detail?uid=${interest?.productUid}`);
          }}
        >
      ...
      <div
        className="interest-table-td2"
        style={{ alignItems: "center", width: 195 }}
      >
        <button
          className="interest-table-delte"
          onClick={() => {
            axios
              .delete(
                `${process.env.REACT_APP_API_URL}/interest?interestUid=${interest.uid}`,
