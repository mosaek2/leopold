...
export default function ShoppingTable({ product }) {
    ...
    return (
        <div className="ShoppingTable">
            <img src={product?.coverUrl} alt="cover" className="st-cover"
                onClick={handleClickCover} />
            <p style={{ marginTop: 22 }}>{product?.productCategory} {product?.color}</p>

            <div style={{ display: "flex", marginTop: 28, alignItems: "baseline" }}>
                <p style={{ fontSize: 24, fontWeight: "bold" }}>{(product?.price * (1 - product?.discountRate))
            ...
            {product?.discountRate !== 0.0
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                    ...
                    <p style={{ color: "#b2b2b2", textDecoration: "line-through" }}>{(product?.price)
